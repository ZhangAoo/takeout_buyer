    ...
    <div class="mui-content">
        <div class="mui-card">
            <ul class="mui-table-view">
                ...
                <li class="mui-table-view-cell">收货地址<span class="spanright">{{data.buyerAddress}}<br/>{{data.buyerName}} {{data.buyerPhone}}</span>
                </li>
                ...
                <li class="mui-table-view-cell">订单状态<span class="spanright orderStatus">{{transformOrderStatus(data.orderStatus)}}</span>
                </li>
                <li class="mui-table-view-cell">支付状态<span class="spanright orderStatus">{{transformPayStatus(data.payStatus)}}</span>
                </li>
                <li class="mui-table-view-cell">
                    <div style="width: 100%;height: 40px;margin-top: 20px;font-size: 13px"
                         v-for="item in data.orderDetailList">
                        <div style="float:left;width: 60px;height: 40px;">
                            <img style="height: 40px" :src="item.productIcon"/>
                        </div>
                        <div style="float:left;height:40px;width: 110px; line-height: 40px; margin-left: 10px;">
                            {{item.productName}}
                        </div>
                        ...
                        <div style="float:right;height:40px;width: 50px; line-height: 40px; margin-left: 20px;font-size: 15px;font-weight: bold">
                            ￥{{item.productPrice*item.productQuantity}}
                        </div>
                    </div>
                </li>
            </ul>
            <button type="button" class="mui-btn mui-btn-block mui-btn-success" v-show="payStatus==0 && orderStatus==0"
                    @click="toPay()">确认付款
            </button>
            <button type="button" class="mui-btn mui-btn-block mui-btn-warning" v-show="orderStatus==0"
